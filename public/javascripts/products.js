...
function sortTable(tableId, columnIndex) {
    ...
    rows.sort((a, b) => {
        ...
        if (columnIndex === 1) {
            aValue = a.cells[columnIndex].querySelector('.product-name')?.textContent.trim() || '';
            bValue = b.cells[columnIndex].querySelector('.product-name')?.textContent.trim() || '';
        }
        // Special handling for price column
        else if (columnIndex === 3) {
            aValue = a.cells[columnIndex].querySelector('.sale-price')?.textContent.trim() || '';
            bValue = b.cells[columnIndex].querySelector('.sale-price')?.textContent.trim() || '';
        } else {
...
function initializeCheckboxes() {
    ...
    const checkboxes = document.querySelectorAll('.product-table .checkbox-cell input[type="checkbox"]');
...
function updateSelectedCount() {
    const checkboxes = document.querySelectorAll('.product-table .checkbox-cell input[type="checkbox"]:checked');
    const count = checkboxes.length - (document.querySelector('th input[type="checkbox"]:checked') ? 1 : 0);
...
function toggleProductVisibility(productId, currentStatus) {
    ...
    showToast(`Sản phẩm đã được ${newStatus === 'visible' ? 'hiển thị' : 'ẩn'} thành công.`);
